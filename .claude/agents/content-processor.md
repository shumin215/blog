...
- `tags`: 아래 키워드 추출 규칙에 따라 생성

**태그 생성 규칙:**

1. WordPress 원본 태그 + 카테고리를 기본으로 수집
2. 본문에서 중요 키워드를 추출해 태그에 추가:
   - 헤딩(`h2`, `h3`)에 등장하는 핵심 단어
   - 본문에서 **bold** 또는 `code` 처리된 단어
   - 3회 이상 반복 등장하는 명사/기술 용어
3. 추출한 키워드를 원본 태그와 합쳐 중복 제거
4. 최종 태그 목록: 영문은 소문자, 한글은 원형 유지, 공백은 `-`로 치환
5. 태그 수: 최소 2개, 최대 10개
